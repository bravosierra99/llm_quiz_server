...
F1 = [
    ...
    mcq("A millisecond is a sliver of which bigger unit of time?",
        ["A second", "An hour", "A day", "A year"], "A second",
        "A millisecond is one-thousandth of a second."),
    tf("1000 milliseconds is the same as 1 second.",
       "True", "Milli means 1/1000, so 1000 ms add up to exactly 1 second."),
    tf("A smaller delay number makes an LED blink SLOWER.",
       "False", "A smaller delay means less waiting, so the blink is FASTER, not slower."),
    ...
    short("How many milliseconds are in one second?",
          "1000", "Milli means one-thousandth, so 1000 ms make 1 second."),
    ...
    short("To make a light blink faster, should the delay number be bigger or smaller?",
          "Smaller", "A smaller delay means less waiting between flashes, so the blink is faster."),
    short("What does 'milli' mean as a fraction?",
          "One-thousandth", "Milli means 1/1000, so a millisecond is 1/1000 of a second."),
...
F2 = [
    ...
    mcq("How many bits are grouped together to make 1 byte?",
        ["8", "2", "10", "16"], "8",
        "8 bits grouped together make 1 byte."),
    ...
    mcq("Reading the binary dot cards from the right, what are the first four values?",
        ["1, 2, 4, 8", "1, 2, 3, 4", "2, 4, 6, 8", "1, 10, 100, 1000"], "1, 2, 4, 8",
        "Each card doubles the one before: 1, then 2, then 4, then 8."),
    ...
    tf("A bit can be 1 (on) or 0 (off).",
       "True", "Those two values, on and off, are the only things a bit can be."),
    tf("8 bits make 1 byte.",
       "True", "A byte is a bundle of 8 bits."),
    tf("In binary, each slot is worth half the one to its right.",
       "False", "Each slot is worth DOUBLE the one to its right: 1, 2, 4, 8, 16..."),
    short("A bit can be on or off. How many different values is that?",
          "2", "A bit is either 1 or 0 — exactly two values."),
...
F3 = [
    ...
    mcq("In the analogRead range, which number is the LOWEST reading?",
        ["0", "1023", "1", "512"], "0",
        "0 is the bottom of the 0-1023 range — the lowest reading."),
    ...
    mcq("Setting LED brightness OUT uses 0-255, but reading a sensor IN uses which range?",
        ["0 to 1023", "0 to 255", "0 to 99", "0 to 12"], "0 to 1023",
        "analogRead reads sensors in 0-1023; analogWrite sets brightness out in 0-255."),
    tf("In the 0-255 brightness range, 0 means fully off.",
       "True", "0 is the lowest value, so it means fully off."),
    tf("analogRead gives back numbers from 0 to 255.",
       "False", "analogRead uses 0 to 1023; the 0-255 range is for PWM brightness out."),
    tf("A bigger sensor reading usually means MORE of what the sensor measures.",
       "True", "Ranges go low-to-high, so a bigger number means more."),
    short("What is the highest number analogRead can give back?",
          "1023", "analogRead reads in the range 0 to 1023, so 1023 is the highest."),
    short("In the PWM brightness range 0-255, which number is fully bright?",
          "255", "255 is the top of the range, so it means fully bright."),
]
